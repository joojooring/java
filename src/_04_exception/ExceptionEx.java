...
// => 자바에서는 예외가 발생하면 예외 클래스로부터 객체를 생성, 해당 객체는 예외 처리시 사용됨
...
//try 블록에서 작성한 코드가 예외 발생시 -> catch 실행 o, finally 실행 o
// => finally 는 항상 실행됨!!
// => 심지어 try 블록과 catch 블록에서 return 문을 사용해도 finally는 항상 실행됨 ( finally 는 생략 가능 )
...
//        case1. 예외 발생 코드
///        System.out.println(divide(6,0)); // 0으로 나누는 코드 : java.lang.ArithmeticException
//        case1. 예외 처리 코드
        try{
...
//            예외 출력 방법
//            방법 3 : 예외가 어디서 발생헀는지 추적한 내용 출력 ( e.printStackTrace(); )
//            e.printStackTrace(); // 가장 마지막에 나오게 됨
        }finally {
...
//        System.out.println(getValueByIdx(numbers, numbers.length)); //ArrayIndexOutOfBoundsException
//        case3. 예외 처리 코드
        try{
...
//        case4. 입력 받을 때 오류
        ...
        } catch (InputMismatchException e){ //import java.util.InputMismatchException; 맨 위에 자동으로 뜸
